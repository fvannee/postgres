...
IndexNext(IndexScanState *node)
{
	...
	if (scandesc == NULL)
	{
		...
		if (node->ioss_SkipPrefixSize > 0)
			index_skip(node->iss_ScanDesc,
					   ((IndexScan *) node->ss.ps.plan)->indexorderdir, node->ioss_SkipPrefixSize,
	 ...
	 */
	while (index_getnext_slot(scandesc, direction, slot, node->ioss_SkipPrefixSize > 0))
	{
...
IndexNextWithReorder(IndexScanState *node)
{
	...
	for (;;)
	{
		 ...
		 */
next_indextuple:
		if (!index_getnext_slot(scandesc, ForwardScanDirection, slot, node->ioss_SkipPrefixSize > 0))
		{
			/*
